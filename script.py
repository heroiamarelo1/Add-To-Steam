...
# --- PATH & REGISTRY SETUP FUNCTIONS (From original generate_reg.py) ---
...
def generate_reg_main():
    ...
    print("--------------------------------------------------------------------------------")
    print("SETUP MODE: Generating Registry File.")
    print("--------------------------------------------------------------------------------")
    ...
    # Save the .reg file in the same directory as this executable
    output_file = os.path.join(get_script_directory(), "add_to_steam_context_menu.reg")
    with open(output_file, "w", encoding="utf-8") as f:
        f.write(reg_content)

    print(f"\nRegistry file '{output_file}' created successfully!")
    print("\nACTION REQUIRED: Double-click this .reg file to activate the context menu.")
    print("Note: If you move this executable, you MUST run this script again and re-apply the .reg file.")

    os.startfile(get_script_directory())
    input("\nPress Enter to exit...")


# --- STEAM SHORTCUT LOGIC FUNCTIONS (From original add_to_steam.py) ---
...
def add_remove_shortcut_main(input_path):
    ...
    # 1. Determine the target executable path
    ext = os.path.splitext(input_path)[1].lower()
    if ext == '.lnk':
        exe_path_for_check = get_shortcut_target(input_path)
    elif ext == '.exe':
    ...
    # Prompt for name
    default_name = os.path.splitext(os.path.basename(input_path if ext == '.lnk' else exe_path_for_check))[0]
    ...
    # Create the VDF entry
    entry = create_entry(exe_path_for_check, name)
    ...
    if not os.path.exists(backup_path):
         # Creating backup *before* reading ensures we have an original copy
